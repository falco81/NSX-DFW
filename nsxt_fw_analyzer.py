...
# Download:  wget -O services-db.csv 'https://www.iana.org/assignments/service-names-port-numbers/service-names-port-numbers.csv'
...
def _load_services_db():
    ...
    if os.path.isfile(SERVICES_DB_FILE):
        ...
        print(f"[SERVICES] For more, download IANA CSV:  wget -O services-db.csv "
              f"'https://www.iana.org/assignments/service-names-port-numbers/"
              f"service-names-port-numbers.csv'", file=sys.stderr)
...
body{background:var(--bg0);color:var(--t1);font-family:'IBM Plex Sans',-apple-system,sans-serif;font-size:14px;line-height:1.5;min-height:100vh}
...
.logo{width:42px;height:42px;background:linear-gradient(135deg,var(--ab),var(--ac));border-radius:var(--r);display:flex;align-items:center;justify-content:center;font-family:'JetBrains Mono',monospace;font-weight:700;font-size:15px;color:#fff;box-shadow:0 0 20px rgba(59,130,246,.15)}
.hdr h1{font-family:'IBM Plex Sans',sans-serif;font-size:22px;font-weight:600;background:linear-gradient(135deg,var(--t1),var(--ac));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hdr-m{font-size:12px;color:var(--tm);font-family:'JetBrains Mono',monospace}
.hdr-r{display:flex;gap:10px}
.btn{padding:8px 16px;border-radius:var(--r);border:1px solid var(--brd);background:var(--bg2);color:var(--t1);font-family:'IBM Plex Sans',sans-serif;font-size:13px;font-weight:500;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center;gap:6px}
...
.sv{font-family:'JetBrains Mono',monospace;font-size:26px;font-weight:700;color:var(--t1)}
.sd{font-size:12px;color:var(--t2);margin-top:6px;font-family:'JetBrains Mono',monospace}
...
.sb input{width:100%;padding:9px 14px 9px 38px;background:var(--bgi);border:1px solid var(--brd);border-radius:var(--r);color:var(--t1);font-family:'JetBrains Mono',monospace;font-size:13px;transition:all .2s;outline:none}
.sb input::placeholder{color:var(--tm)}
.sb input:focus{border-color:var(--ab);box-shadow:0 0 20px rgba(59,130,246,.15)}
.rc{font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--tm);white-space:nowrap;padding:9px 0}
.rc b{color:var(--ac)}
.tw{border:1px solid var(--brd);border-radius:var(--rl);overflow:hidden;background:var(--bg1)}
.ts{overflow-x:auto;max-height:calc(100vh - 360px);overflow-y:auto}
...
tbody td{padding:8px 14px;font-family:'JetBrains Mono',monospace;font-size:12px;white-space:nowrap;color:var(--t1)}
...
.badge{display:inline-block;padding:2px 10px;border-radius:20px;font-size:11px;font-weight:600;letter-spacing:.5px;font-family:'JetBrains Mono',monospace}
...
.pg button{padding:5px 12px;border-radius:var(--r);border:1px solid var(--brd);background:var(--bgi);color:var(--t2);font-family:'JetBrains Mono',monospace;font-size:12px;cursor:pointer;transition:all .15s}
...
.pg .pi{font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--tm);margin:0 8px}
.pss{padding:5px 8px;background:var(--bgi);border:1px solid var(--brd);border-radius:var(--r);color:var(--t2);font-family:'JetBrains Mono',monospace;font-size:12px;margin-left:12px;cursor:pointer;outline:none}
...
.tr_{display:flex;justify-content:space-between;align-items:center;padding:4px 0;font-family:'JetBrains Mono',monospace;font-size:12px}
...
.ct{background:none;border:none;color:var(--t2);font-family:'IBM Plex Sans',sans-serif;font-size:13px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:6px;padding:8px 0;margin-bottom:8px}
...
.col-popup .cp-filter input{width:100%;padding:6px 10px;background:var(--bgi);border:1px solid var(--brd);border-radius:var(--r);color:var(--t1);font-family:'JetBrains Mono',monospace;font-size:12px;outline:none;transition:border-color .2s}
.col-popup .cp-filter input:focus{border-color:var(--ab)}
.col-popup .cp-filter input::placeholder{color:var(--tm)}
.col-popup .cp-hint{padding:2px 10px 6px;font-size:10px;color:var(--tm);font-family:'JetBrains Mono',monospace;line-height:1.4}
...
.col-panel-dropdown label{display:flex;align-items:center;gap:8px;padding:5px 14px;font-size:12px;color:var(--t2);cursor:pointer;transition:background .1s;font-family:'JetBrains Mono',monospace}
...
.col-panel-dropdown .cp-actions button{flex:1;padding:4px 8px;border:1px solid var(--brd);border-radius:var(--r);background:var(--bgi);color:var(--t2);font-size:11px;cursor:pointer;transition:all .15s;font-family:'IBM Plex Sans',sans-serif}
...
.af-tag{display:inline-flex;align-items:center;gap:4px;padding:3px 10px;background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.3);border-radius:20px;font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--ao)}
...
function render(){
  ...
  if(sl.length===0){
    ...
    for(const r of sl){
      h+='<tr>';
      for(const ci of visCols)h+=fC(r[ci],ci,q);
      h+='</tr>';
    }
    tb.innerHTML=h;
  }
...
function togglePopup(ci,thEl){
  ...
  h+='<div class="cp-hint">* = any &nbsp; ? = one char &nbsp; ! = exclude<br>/regex/i &nbsp; Examples: !DROP &nbsp; !10.12.* &nbsp; !/^PASS$/</div>';
  if(colFilters[ci]){
...
def write_html(recs, path, dns_map=None, sort_by="src_ip", translate=True):
    ...
    json_rows = []
    for r in rows:
        json_rows.append([r.get(c, "") for c in cols])
    ...
    <div class="sc"><div class="sl">Unique Ports</div><div class="sv" style="color:var(--ao)">{len(pc):,}</div><div class="sd">destination ports</div></div>
    '''
...
================================================================================
  ...
  Sources (in priority order):
    ...
    1) IANA CSV file (services-db.csv) next to this script.
       Most comprehensive mapping (~6000 services). Download once:
